...
if (Vars::$USER_RIGHTS == 3 || Vars::$USER_RIGHTS >= 6) {
    ...
    if (isset($_GET['delvote']) && !empty($_GET['vote'])) {
        ...
        if ($totalvote != 0) {
            if (isset($_GET['yes'])) {
                ...
                echo '<div class="rmenu"><p>' . $lng_forum['voting_variant_warning'] . '<br />' .
                     '<a href="index.php?act=editvote&amp;id=' . Vars::$ID . '&amp;vote=' . $vote . '&amp;delvote&amp;yes">' . Vars::$LNG['delete'] . '</a><br />' .
                     '<a href="' . htmlspecialchars(getenv("HTTP_REFERER")) . '">' . Vars::$LNG['cancel'] . '</a></p></div>';
            }
        } else {
            header('location: ?act=editvote&id=' . Vars::$ID . '');
        }
        ...
        echo '<div class="gmenu"><p>' . $lng_forum['voting_changed'] . '<br /><a href="index.php?id=' . Vars::$ID . '">' . Vars::$LNG['continue'] . '</a></p></div>';
    } else {
        /*
        -----------------------------------------------------------------
        ...
        */
        ...
        echo '<div class="phdr"><a href="index.php?id=' . Vars::$ID . '"><b>' . Vars::$LNG['forum'] . '</b></a> | ' . $lng_forum['edit_vote'] . '</div>' .
        ...
        if ($countvote < 20) {
            ...
            for ($vote = $i; $vote < $_POST['count_vote']; $vote++) {
                echo 'Ответ ' . ($vote + 1) . '(max. 50): <br/><input type="text" name="' . $vote . '" value="' . Validate::filterString($_POST[$vote]) . '"/><br/>';
            }
            echo '<input type="hidden" name="count_vote" value="' . abs(intval($_POST['count_vote'])) . '"/>' . ($_POST['count_vote'] < 20 ? '<input type="submit" name="plus" value="' . Vars::$LNG['add'] . '"/>' : '')
        ...
        echo '</p></div><div class="gmenu">' .
             ...
             '</div></form>' .
             '<div class="phdr"><a href="index.php?id=' . Vars::$ID . '">' . Vars::$LNG['cancel'] . '</a></div>';
    }
}