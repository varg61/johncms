...
$do = array('new', 'who', 'addfile', 'file', 'moders', 'per', 'fmoder', 'ren',
    'deltema', 'vip', 'close', 'delpost', 'editpost', 'nt', 'tema', 'loadtem', 'say',
    'post', 'read', 'faq', 'trans');
if (in_array($act, $do)) {
    ...
    if ($id) {
        ...
        switch ($tip) {
            case "f":
                ...
                // Список разделов                                        //
                ////////////////////////////////////////////////////////////
                echo '<div class="phdr"><div><img src="../images/nnew.gif" width="30" height="15"/>&nbsp;<a href="index.php?act=new"><small>Непрочитанное&nbsp;(' .
                    forum_new() . ')</small></a></div><div style="height:18px"><img src="../images/nstart.gif" width="15" height="15"/>&nbsp;<a href="index.php">Форум</a></div><img src="../images/nfinal.gif" width="30" height="15"/>&nbsp;<b>' .
                    $type1['text'] . '</b></div>';
                $q1 = mysql_query("SELECT `id`, `text` FROM `forum` WHERE `type`='r' AND `refid`='" .
                    $id . "' ORDER BY `realid`;");
                $colraz2 = mysql_num_rows($q1);
                $i = 0;
                while ($mass1 = mysql_fetch_array($q1)) {
                    $coltem = mysql_query("select id, time from `forum` where type='t' and moder='1' and refid='" .
                        $mass1['id'] . "' order by time desc;");
                    $coltem1 = mysql_num_rows($coltem);
                    echo '<div class="menu"><a href="?id=' . $mass1['id'] . '">' . $mass1['text'] .
                        '</a>';
                    if ($coltem1 > 0) {
                        echo " [$coltem1]";
                    }
            ...
            case "r":
                ...
                ////////////////////////////////////////////////////////////
                if ($dostsadm == 1) {
                    $qz = mysql_query("SELECT COUNT(*) FROM `forum` WHERE `type`='t' AND `refid`='" .
                        $id . "' AND `moder`='1' ;");
                } else {
                    $qz = mysql_query("SELECT COUNT(*) FROM `forum` WHERE `type`='t' AND `close`!='1' AND `moder`='1' AND `refid`='" .
                        $id . "';");
                }
                ...
                echo '<div class="phdr"><div><img src="../images/nnew.gif" width="30" height="15"/>&nbsp;<a href="index.php?act=new"><small>Непрочитанное&nbsp;(' .
                    forum_new() . ')</small></a></div><div style="height:18px"><img src="../images/nstart.gif" width="15" height="15"/>&nbsp;<a href="index.php">Форум</a><small> &gt; <a href="index.php?id=' .
                    $type1['refid'] . '">' . $forum1['text'] .
                    '</a></small></div><img src="../images/nfinal.gif" width="30" height="15"/>&nbsp;<b>' .
                    $type1['text'] . '</b></div>';

                if ($user_id && !$ban['1'] && !$ban['11']) {
                    echo '<div class="gmenu"><a href="index.php?act=nt&amp;id=' . $id .
                        '">Новая тема</a></div>';
                }
                if ($dostsadm == 1) {
                    $q1 = mysql_query("SELECT `id`, `from`, `time`, `vip`, `close`, `edit`, `text` FROM `forum` WHERE `type`='t' AND `refid`='" .
                        $id . "' AND `moder`='1'  ORDER BY `vip` DESC, `time` DESC LIMIT " . $start .
                        "," . $kmess . ";");
                } else {
                    $q1 = mysql_query("SELECT `id`, `from`, `time`, `vip`, `close`, `edit`, `text` FROM `forum` WHERE `type`='t' AND `close`!='1' AND `moder`='1' AND `refid`='" .
                        $id . "'  ORDER BY `vip` DESC, `time` DESC LIMIT " . $start . "," . $kmess . ";");
                }
                $i = 0;
                while ($mass = mysql_fetch_array($q1)) {
                    $colmes = mysql_query("SELECT `id` FROM `forum` WHERE `type` = 'm' AND `close` != '1' AND `refid` = '" .
                        $mass['id'] . "'ORDER BY time DESC;");
                    $nikuser = mysql_query("SELECT `from` FROM `forum` WHERE `type` = 'm' AND `close` != '1' AND `refid` = '" .
                        $mass['id'] . "'ORDER BY time DESC LIMIT 1;");
                    ...
                    if ($mass['vip'] == 1) {
                        ...
                        $np = mysql_query("SELECT `id` FROM `forum` WHERE `type`='l' AND `time`>='" . $mass['time'] .
                            "' AND `refid`='" . $mass['id'] . "' and `from`='" . $login . "';");
                    ...
                    echo ' <font color="#777777">' . date("d.m.y / H:i", $mass['time']) .
                        "</font></div></div>";
                    ++$i;
                }
                ...
                if ($coltem > $kmess) {
                    ...
                    $pagenav = array('address' => 'index.php?id=' . $id, 'total' => $coltem, 'numpr' =>
                        $kmess, 'page' => $page);
                    pagenav($pagenav);
                    echo '</p>';
                    echo "<form action='index.php'>Перейти к странице:<br/><input type='hidden' name='id' value='" .
                        $id . "'/><input type='text' name='page' title='Введите номер страницы'/><br/><input type='submit' title='Нажмите для перехода' value='Go!'/></form>";
                }
                break;
            ...
            case "t":
                ...
                if ($user_id) {
                    ...
                    $req = mysql_query("SELECT * FROM `cms_forum_rdm` WHERE `topic_id`='" . $id .
                        "' AND `user_id`='" . $user_id . "';");
                ...
                if ($dostsadm != 1 && $type1['close'] == 1) {
                    ...
                    echo "<font color='#FF0000'>Тема удалена!</font><br/><a href='?id=" . $type1['refid'] .
                        "'>В раздел</a><br/>";
                    require_once ("../incfiles/end.php");
                    exit;
                }
                if ($dostsadm == 1) {
                    ...
                    $qz = mysql_query("select `id` from `forum` where type='m' and close!='1' and refid='" .
                        $id . "'  ;");
                }
                ...
                $start = $page * $kmess - $kmess;
                if (((empty($_SESSION['uid'])) && (!empty($_SESSION['uppost'])) && ($_SESSION['uppost'] ==
                    1)) || ((!empty($_SESSION['uid'])) && $upfp == 1)) {
                    if ($dostsadm == 1) {
                        $q1 = mysql_query("SELECT * FROM `forum` WHERE type='m' AND refid='" . $id .
                            "'  ORDER BY time DESC LIMIT " . $start . ", " . $kmess . " ;");
                    } else {
                        $q1 = mysql_query("SELECT * FROM `forum` WHERE type='m' AND close!='1' AND refid='" .
                            $id . "'  ORDER BY time DESC LIMIT " . $start . ", " . $kmess . "  ;");
                    }
                } else {
                    if ($dostsadm == 1) {
                        $q1 = mysql_query("SELECT * FROM `forum` WHERE type='m' AND refid='" . $id .
                            "'  ORDER BY time LIMIT " . $start . ", " . $kmess . " ;");
                    } else {
                        $q1 = mysql_query("SELECT * FROM `forum` WHERE type='m' AND close!='1' AND refid='" .
                            $id . "'  ORDER BY time LIMIT " . $start . ", " . $kmess . " ;");
                    }
                }
                ...
                echo '<div class="phdr"><div><img src="../images/nnew.gif" width="30" height="15"/>&nbsp;<a href="index.php?act=new"><small>Непрочитанное&nbsp;(' .
                    forum_new() . ')</small></a></div><div style="height:18px"><img src="../images/nstart.gif" width="15" height="15"/>&nbsp;<a href="index.php">Форум</a><small> &gt; <a href="index.php?id=' .
                    $frm['id'] . '">' . $frm['text'] .
                    '</a></small></div><img src="../images/nfinal.gif" width="30" height="15"/>&nbsp;<b><a href="index.php?id=' .
                    $razd['id'] . '">' . $razd['text'] . '</a></b></div>';
                ...
                if ($type1['edit'] != 1 && $_SESSION['uid'] != "" && $upfp == 1) {
                    if ($datauser['farea'] == 1 && $datauser['postforum'] >= 1) {
                        echo "<div class='e'>Написать<br/><form action='index.php?act=say&amp;id=" . $id .
                            "' method='post' enctype='multipart/form-data'><textarea cols='20' rows='2' title='Введите текст сообщения' name='msg'></textarea><br/>";
                ...
                while ($mass = mysql_fetch_array($q1)) {
                    if ($i >= 0 && $i < $colmes) {
                        ...
                        $uz = mysql_query("SELECT `id`, 'from', `sex`, `rights`, `lastdate`, `dayb`, `status`, `datereg` FROM `users` WHERE `name`='" .
                            $mass['from'] . "';");
                        $mass1 = mysql_fetch_array($uz);
                        echo "$div";
                        echo $mass1['datereg'] > $realtime - 86400 ?
                        ...
                        if ((!empty($_SESSION['uid'])) && ($_SESSION['uid'] != $mass1['id'])) {
                            echo "<a href='index.php?act=say&amp;id=" . $mass['id'] . "'><b>$mass[from]</b></a> <a href='index.php?act=say&amp;id=" .
                                $mass['id'] . "&amp;cyt'> [ц]</a>";
                        } else {
                            echo "<b>$mass[from]</b>";
                        }
                        ...
                        if (!empty($mass1['status']))
                            echo '<div class="status"><img src="../images/star.gif" alt=""/>&nbsp;' . $mass1['status'] .
                                '</div>';
                        ...
                        if ($offsm != 1 && $offgr != 1) {
                            $text = smiles($text);
                            $text = smilescat($text);
                            if ($mass['from'] == nickadmina || $mass['from'] == nickadmina2 || $mass1['rights'] >=
                                1) {
                                $text = smilesadm($text);
                            }
                        }
                        ...
                        if (mb_strlen($text) >= 500) {
                            ...
                            echo $text . '...<br /><a href="index.php?act=post&amp;s=' . $page . '&amp;id=' .
                                $mass['id'] . '">весь пост &gt;&gt;</a>';
                        } else {
                        ...
                        if (!empty($mass['attach'])) {
                            ...
                            echo "<br /><font color='#999999'>Прикреплённый файл:<br /><a href='index.php?act=file&amp;id=" .
                                $mass['id'] . "'>$mass[attach]</a> ($fls кб.)<br/>";
                            echo 'Скачано: ' . $mass['dlcount'] . ' раз.</font>';
                        }
                        $lp = mysql_query("select `from`, `id` from `forum` where type='m' and refid='" .
                            $id . "'  order by time desc LIMIT 1;");
                        $arr1 = mysql_fetch_array($lp);
                        $tpp = $realtime - 300;
                        if (($dostfmod == 1) || (($arr1['from'] == $login) && ($arr1['id'] == $mass['id']) &&
                            ($mass['time'] > $tpp))) {
                            echo "<br /><a href='index.php?act=editpost&amp;id=" . $mass['id'] .
                                "'>Изменить</a>";
                        }
                        if ($dostfmod == 1) {
                ...
                if ($colmes > $kmess) {
                    ...
                    echo "<form action='?'>Перейти к странице:<br/><input type='hidden' name='id' value='" .
                        $id . "'/><input type='text' name='page' title='Введите номер страницы'/><br/><input type='submit' title='Нажмите для перехода' value='Go!'/></form></p>";
                }
                if ($dostfmod == 1) {
                    echo '<div class="func">';
                    if ($type1['moder'] != 1) {
                    ...
                    echo "<a href='index.php?act=ren&amp;id=" . $id .
                        "'>Переименовать тему</a><br/>";
                    if ($type1['edit'] == 1) {
                        echo "<a href='index.php?act=close&amp;id=" . $id . "'>Открыть тему</a><br/>";
                    } else {
                        echo "<a href='index.php?act=close&amp;id=" . $id .
                            "&amp;closed'>Закрыть тему</a><br/>";
                    }
                    ...
                    echo "<br/><a href='index.php?act=per&amp;id=" . $id .
                        "'>Переместить тему</a></div><br />";
                }
                if (!empty($_SESSION['uid'])) {
                    echo "<a href='index.php?act=who&amp;id=" . $id . "'>Кто здесь?(" . wfrm($id) .
                        ")</a><br/>";
                }
        ...
        // Список Форумов                                         //
        ////////////////////////////////////////////////////////////
        echo '<div class="phdr"><img src="../images/nnew.gif" width="30" height="15"/>&nbsp;<a href="index.php?act=new"><small>Непрочитанное&nbsp;(' .
            forum_new() . ')</small></a><br /><img src="../images/nstart.gif" width="15" height="15"/>&nbsp;<b>Форум</b></div>';
        $q = mysql_query("SELECT `id`, `text` FROM `forum` WHERE `type`='f' ORDER BY `realid`;");
        while ($mass = mysql_fetch_array($q)) {
            $colraz = mysql_query("SELECT COUNT(*) FROM `forum` WHERE `type`='r' and `refid`='" .
                $mass['id'] . "';");
            $colraz1 = mysql_result($colraz, 0);
            echo '<div class="menu"><a href="index.php?id=' . $mass['id'] . '">' . $mass['text'] .
                '</a> [' . $colraz1 . ']</div>';
        }
        if ($user_id) {
            echo '<div class="bmenu"><a href="index.php?act=who">Кто в форуме</a>(' . wfrm() .
                ')</div>';
        } else {
    ...
    if (empty($_SESSION['uid'])) {
        if ((empty($_SESSION['uppost'])) || ($_SESSION['uppost'] == 0)) {
            echo "<a href='index.php?id=" . $id . "&amp;page=" . $page .
                "&amp;newup'>Новые вверху</a><br/>";
        } else {
            echo "<a href='index.php?id=" . $id . "&amp;page=" . $page .
                "&amp;newdown'>Новые внизу</a><br/>";
        }
    }
