...
FROM `forum_vote_us` LEFT JOIN `users` ON `forum_vote_us`.`user` = `users`.`id` WHERE `forum_vote_us`.`topic`='$id' ORDER BY `time` DESC LIMIT " . $start . "," . $kmess);
...
if ($colmes == 0) echo '<div class="menu">В этом опросе пока никто не участвовал!</div>';
echo '<div class="bmenu">Всего: ' . $colmes . '</div>';
    if ($colmes > $kmess)
                {
                    echo '<p>' . pagenav('index.php?act=users&amp;id=' . $id . '&amp;', $start, $colmes, $kmess) . '</p>';
                    echo '<p><form action="index.php" method="get"><input type="hidden" name="act" value="users"/><input type="hidden" name="id" value="' . $id . '"/><input type="text" name="page" size="2"/><input type="submit" value="К странице &gt;&gt;"/></form></p>';
                }
