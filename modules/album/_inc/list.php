...
if ($total) {
    ...
    while ($res = mysql_fetch_assoc($req)) {
        ...
        if ($user['user_id'] == Vars::$USER_ID || Vars::$USER_RIGHTS >= 6 || !empty($res['description'])) {
            $menu = array (
                '<a href="album.php?act=sort&amp;mod=up&amp;al=' . $res['id'] . '&amp;user=' . $user['user_id'] . '">' . lng('up') . '</a>',
                '<a href="album.php?act=sort&amp;mod=down&amp;al=' . $res['id'] . '&amp;user=' . $user['user_id'] . '">' . lng('down') . '</a>',
                '<a href="album.php?act=edit&amp;al=' . $res['id'] . '&amp;user=' . $user['user_id'] . '">' . lng('edit') . '</a>',
                '<a href="album.php?act=delete&amp;al=' . $res['id'] . '&amp;user=' . $user['user_id'] . '">' . lng('delete') . '</a>'
            );
            echo '<div class="sub">' .
