...
if ($img && $user['user_id'] == Vars::$USER_ID || Vars::$USER_RIGHTS >= 6) {
    ...
    if (mysql_num_rows($req)) {
        ...
        echo '<div class="phdr"><a href="album.php?act=show&amp;al=' . $image['album_id'] . '&amp;user=' . $user['user_id'] . '"><b>' . lng('photo_album') . '</b></a> | ' . lng('image_move') . '</div>';
        if (isset($_POST['submit'])) {
            ...
            if (mysql_num_rows($req)) {
                ...
                echo '</select></p>' .
                     ...
                     '</div></form>' .
                     '<div class="phdr"><a href="album.php?act=show&amp;al=' . $image['album_id'] . '&amp;user=' . $user['user_id'] . '">' . lng('cancel') . '</a></div>';
            } else {
                echo Functions::displayError(lng('image_move_error'), '<a href="album.php?act=list&amp;user=' . $user['user_id'] . '">' . lng('continue') . '</a>');
            }
        }
    } else {
        echo Functions::displayError(lng('error_wrong_data'));
