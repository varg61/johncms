...
if ($img && $user['id'] == Vars::$USER_ID || Vars::$USER_RIGHTS >= 6) {
    ...
    if (mysql_num_rows($req)) {
        ...
        echo '<div class="phdr"><a href="' . Vars::$URI . '?act=show&amp;al=' . $image['album_id'] . '&amp;user=' . $user['id'] . '"><b>' . lng('photo_album') . '</b></a> | ' . lng('image_move') . '</div>';
        if (isset($_POST['submit'])) {
            ...
            if (mysql_num_rows($req)) {
                ...
                echo '</select></p>' .
                     ...
                     '</div></form>' .
                     '<div class="phdr"><a href="' . Vars::$URI . '?act=show&amp;al=' . $image['album_id'] . '&amp;user=' . $user['id'] . '">' . lng('cancel') . '</a></div>';
            } else {
                echo Functions::displayError(lng('image_move_error'), '<a href="' . Vars::$URI . '?act=list&amp;user=' . $user['id'] . '">' . lng('continue') . '</a>');
            }
        }
    } else {
        echo Functions::displayError(lng('error_wrong_data'));
