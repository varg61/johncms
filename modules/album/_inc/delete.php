...
if ($al && $user['id'] == Vars::$USER_ID || Vars::$USER_RIGHTS >= 6) {
    ...
    if (mysql_num_rows($req_a)) {
        ...
        echo '<div class="phdr"><a href="' . Vars::$URI . '?act=list&amp;user=' . $user['id'] . '"><b>' . __('photo_album') . '</b></a> | ' . __('delete') . '</div>';
        if (isset($_POST['submit'])) {
            ...
            echo '<div class="rmenu"><form action="' . Vars::$URI . '?act=delete&amp;al=' . $al . '&amp;user=' . $user['id'] . '" method="post">' .
                ...
                '</form></div>' .
                '<div class="phdr"><a href="' . Vars::$URI . '?act=list&amp;user=' . $user['id'] . '">' . __('cancel') . '</a></div>';
        }
    } else {
