...
switch (Vars::$ACT) {
    case 'sent':
        ...
        */
        ...
        if (!$error) {
            ...
            $mail = lng('restore_help1') . ', ' . $res['nickname'] . "\r\n" . lng('restore_help2') . ' ' . Vars::$HOME_URL . "\r\n";
            $mail .= lng('restore_help3') . ": \r\n" . Vars::$HOME_URL . "/users/skl.php?act=set&id=" . $res['id'] . "&code=" . $check_code . "\n\n";
            $mail .= lng('restore_help4') . "\r\n";
            ...
            if (mail($res['mail'], $subject, $mail, $adds)) {
                ...
                echo '<div class="gmenu"><p>' . lng('restore_help6') . '</p></div>';
            } else {
                echo '<div class="rmenu"><p>' . lng('error_email_sent') . '</p></div>';
            }
        } else {
    ...
    case 'set':
        ...
        */
        ...
        if (!$error) {
            ...
            $mail = lng('restore_help1') . ', ' . $res['nickname'] . "\r\n" . lng('restore_help8') . ' ' . Vars::$HOME_URL . "\r\n";
            $mail .= lng('your_new_password') . ": $pass\r\n";
            $mail .= lng('restore_help7');
            $adds = "From: <" . Vars::$SYSTEM_SET['email'] . ">\n";
            $adds .= "Content-Type: text/plain; charset=\"utf-8\"\r\n";
            if (mail($res['mail'], $subject, $mail, $adds)) {
                mysql_query("UPDATE `users` SET `rest_code` = '', `password` = '" . md5(md5($pass)) . "' WHERE `id` = " . Vars::$ID);
                echo '<div class="phdr">' . lng('change_password') . '</div>';
                echo '<div class="gmenu"><p>' . lng('change_password_conf') . '</p></div>';
            } else {
                echo '<div class="rmenu"><p>' . lng('error_email_sent') . '</p></div>';
            }
        } else {
    ...
    default:
        ...
        -----------------------------------------------------------------
        */
        echo '<div class="phdr"><b>' . lng('password_restore') . '</b></div>';
        echo '<div class="menu"><form action="skl.php?act=sent" method="post">';
        echo '<p>' . lng('your_login') . ':<br/><input type="text" name="nick" /><br/>';
        echo lng('your_email') . ':<br/><input type="text" name="email" /></p>';
        echo '<p><img src="../captcha.php?r=' . mt_rand(1000, 9999) . '" alt="' . lng('captcha') . '"/><br />';
        echo '<input type="text" size="5" maxlength="5"  name="code"/>&#160;' . lng('enter_code') . '</p>';
        echo '<p><input type="submit" value="' . lng('sent') . '"/></p></form></div>';
        echo '<div class="phdr"><small>' . lng('restore_help') . '</small></div>';
        break;
}