<div class="phdr">
    <a href="<?= Vars::$URI ?>?act=edit&amp;user=<?= $this->user['id'] ?>"><b><?= ($this->user['id'] != Vars::$USER_ID ? lng('profile') : lng('my_profile')) ?></b></a> | <?= lng('edit') ?>
</div>
<div class="gmenu">
    <div class="formblock">
    ...
    <ul class="formblock">
        ...
        <li><a href="<?= Vars::$URI ?>?act=edit&amp;mod=upload_avatar&amp;user=<?= $this->user['id'] ?>"><?= lng('upload_image') ?></a></li>
        <?php endif; ?>
        <?php if ($this->setUsers['upload_animation'] || Vars::$USER_RIGHTS >= 7) : ?>
        <li><a href="<?= Vars::$URI ?>?act=edit&amp;mod=upload_animation&amp;user=<?= $this->user['id'] ?>"><?= lng('upload_animation') ?></a></li>
        <?php endif; ?>
        <?php if ($this->user['id'] == Vars::$USER_ID) : ?>
        <li><a href="<?= Vars::$HOME_URL ?>/avatars"><?= lng('select_in_catalog') ?></a></li>
        <?php endif; ?>
        <?php if (isset($this->avatar)) : ?>
        <li><a href="<?= Vars::$URI ?>?act=edit&amp;mod=delete_avatar&amp;user=<?= $this->user['id'] ?>"><?= lng('delete') ?></a></li>
        <?php endif; ?>
    </ul>
</div>
<div class="phdr">
