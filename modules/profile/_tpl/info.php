...
<div class="list2">
    <div class="formblock">
        ...
        <ul>
            ...
            <?php endif; ?>
            <li><span class="gray"><?= lng('name') ?>:</span> <?= (empty($this->user['imname']) ? '' : Validate::checkout($this->user['imname'])) ?></li>
            <li><span class="gray"><?= lng('birthday') ?>:</span></li>
            <li><span class="gray"><?= lng('live') ?>:</span> <?= (empty($this->user['live']) ? '' : Validate::checkout($this->user['live'])) ?></li>
            <li><span class="gray"><?= lng('about') ?>:</span> <?= (empty($this->user['about']) ? '' : '<br />' . Functions::smileys(Validate::checkout($this->user['about'], 1, 1))) ?></li>
        </ul>
    </div>
    <div class="formblock">
        <label><?= lng('communication') ?></label>
        <ul>
            <li><span class="gray"><?= lng('phone_number') ?>:</span> <?= (empty($this->user['tel']) ? '' : Validate::checkout($this->user['tel'])) ?></li>
            <li><span class="gray">E-mail:</span>
                ...
                <?= Validate::checkout($this->user['email']) . ($this->user['mailvis'] ? '' : '<span class="gray"> [' . lng('hidden') . ']</span>') ?>
                <?php endif; ?>
            </li>
            <li><span class="gray">ICQ:</span> <?= $this->user['icq'] ?></li>
            <li><span class="gray">Skype:</span> <?= Validate::checkout($this->user['skype']) ?></li>
            <li><span class="gray"><?= lng('site') ?>:</span> <?= Validate::checkout($this->user['siteurl'], 0, 1) ?></li>
        </ul>
    </div>
    <div class="formblock">
