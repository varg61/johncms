...
if (Vars::$USER_ID) {
    ...
    if (!$bookmark) {
        echo '<a href="' . Vars::$URI . '?act=view&amp;id=' . Vars::$ID . '&amp;addBookmark">' . lng('add_favorite') . '</a>';
    } else {
        echo '<a href="' . Vars::$URI . '?act=view&amp;id=' . Vars::$ID . '&amp;delBookmark">' . lng('delete_favorite') . '</a>';
    }
    echo '</div>';
}
/*
...
if ($res_down['about'])
    echo '<b>' . lng('dir_desc') . ':</b> ' . Validate::checkout($res_down['about'], 1, 1);
...
echo ': <b><span class="green">' . $file_rate[0] . '</span>/<span class="red">' . $file_rate[1] . '</span></b><br />' .
    '<img src="' . Vars::$HOME_URL . '/assets/misc/rating.php?img=' . $sum . '" alt="' . lng('rating') . '" />';
/*
-----------------------------------------------------------------
...
if (Vars::$SYSTEM_SET['mod_down_comm'] || Vars::$USER_RIGHTS >= 7)
    echo '<div class="sub"></div><a href="' . Vars::$URI . '?act=comments&amp;id=' . $res_down['id'] . '">' . lng('comments') . '</a> (' . $res_down['total'] . ')';
echo '</div>';
/*
-----------------------------------------------------------------
