...
if ($edit) {
	...
	*/
    ...
    if ($name_link && mysql_num_rows($req_file_more) && isset($_POST['submit'])) {
        ...
        '</div><div class="phdr"><a href="' . Vars::$URI . '?act=files_more&amp;id=' . Vars::$ID . '">' . __('back') . '</a></div>';
    }
} else if ($del) {
	/*
	-----------------------------------------------------------------
	...
	*/
    ...
    if ($link_file) {
        ...
        if ($error) {
            $error[] = '<a href="' . Vars::$URI . '?act=files_more&amp;id=' . Vars::$ID . '">' . __('repeat') . '</a>';
    ...
    if ($do_file) {
		...
		if ($error) {
            ...
            if ($up_file == TRUE) {
                ...
                '<a href="' . Vars::$URI. '?act=files_more&amp;id=' . Vars::$ID. '">' . __('upload_file_more') . '</a> | <a href="' . Vars::$URI. '?id=' . Vars::$ID. '&amp;act=view">' . __('back') . '</a></div>';
                ...
                echo '<div class="rmenu">' . __('upload_file_no') . '<br /><a href="' . Vars::$URI. '?act=files_more&amp;id=' . Vars::$ID . '">' . __('repeat') . '</a></div>';
		}
    } else
        echo '<div class="rmenu">' . __('upload_file_no') . '<br /><a href="' . Vars::$URI. '?act=files_more&amp;id=' . Vars::$ID . '">' . __('repeat') . '</a></div>';
} else {
	...
	*/
	...
	if ($total_file) {
        while ($res_file_more = mysql_fetch_assoc($req_file_more)) {
			...
			'<a href="' . Vars::$URI . '?act=files_more&amp;id=' . Vars::$ID . '&amp;edit=' . $res_file_more['id'] . '">' . __('edit') . '</a> | ' .
            '<span class="red"><a href="' . Vars::$URI . '?act=files_more&amp;id=' . Vars::$ID . '&amp;del=' . $res_file_more['id'] . '">' . __('delete') . '</a></span></div></div>';
        }
