                    ...
                    foreach ($arr_scan_dir as $val) {
                        if (!in_array($val, $array_dowm)) {
                            if (is_dir($val)) {
                                ...
                                mysql_query("INSERT INTO `cms_download_files` SET `refid`='$refid', `dir`='$dir', `time`='" . time() . "', `name`='$name', `type` = '1', `field`='0', `rus_name`='$name'");
                                ...
                                if (preg_match("/^file([0-9]+)_/", $name)) {
                                    if (!in_array($name, $array_more)) {
                                        ...
                                        $size = filesize($val);
                                        mysql_query("INSERT INTO `cms_download_more` SET `refid`='$refid', `time`='" . time() ."',`name`='$name', `rus_name` = '$name_link',`size`='$size'");
                                        ++$i_two;
                                    }
                                } else {
