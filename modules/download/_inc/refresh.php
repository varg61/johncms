...
if (Vars::$USER_RIGHTS == 4 || Vars::$USER_RIGHTS >= 6) {
    ...
    while (($file1 = readdir($dropen))) {
        if ($file1 != "." && $file1 != ".." && $file1 != "index.php") {
            ...
            if (!in_array($file1, $drt)) {
                if (is_dir($loadroot . DIRECTORY_SEPARATOR . $file1)) {
                    mysql_query("INSERT INTO `download` SET
                        `adres` = '" . mysql_real_escape_string($loadroot) . "',
                        `time` = '" . time() . "',
                        `name` = '" . mysql_real_escape_string($file1) . "',
                        `type` = 'cat',
                        `text` = '" . mysql_real_escape_string($file1) . "'
                    ") or die(mysql_error());
                }
            }
        }
    ...
    while ($obn1 = mysql_fetch_array($obn)) {
        $dirop = $obn1['adres'] . DIRECTORY_SEPARATOR . $obn1['name'];
        if (is_dir($dirop)) {
            $diropen = opendir($dirop);
            while ($file = readdir($diropen)) {
                if ($file != "." && $file != ".." && $file != "index.php") {
                    $pap = $obn1['adres'] . DIRECTORY_SEPARATOR . $obn1['name'];
                    ...
                    if (!in_array($file, $fod)) {
                        if (is_dir($dirop . DIRECTORY_SEPARATOR . $file)) {
                            mysql_query("insert into `download` values(0,'" . $obn1['id'] . "','" . $pap . "','" . time() . "','" . $file . "','cat','','','','" . $file . "','');");
                        }
                        if (is_file($dirop . DIRECTORY_SEPARATOR . $file)) {
    ...
    while ($deld1 = mysql_fetch_array($deld)) {
        if (!is_dir($deld1['adres'] . DIRECTORY_SEPARATOR . $deld1['name'])) {
    ...
    while ($delf1 = mysql_fetch_array($delf)) {
        if (!is_file($delf1['adres'] . DIRECTORY_SEPARATOR . $delf1['name'])) {
            mysql_query("delete from `download` where id='" . $delf1['id'] . "' LIMIT 1;");
            $idf = $idf + 1;
        }
    }
    echo '<h3>' . lng('refreshed') . "</h3>" . lng('added') . " $totald " . lng('folders') . " и $totalf " . lng('files') . "<br/>
" . lng('deleted') . " $idd " . lng('folders') . " и $idf " . lng('files') . "<br/>";
    if ($totald != 0 || $totalf != 0) {
        echo "<a href='?act=refresh'>" . lng('refresh_continue') . "</a><br/>";
    }
}
