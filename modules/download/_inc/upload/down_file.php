    ...
    if (($res['field'] && Vars::$USER_ID) || (Vars::$USER_RIGHTS == 4 || Vars::$USER_RIGHTS >= 6)) {
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($do_file) {
				...
				if ($error) {
                    $error[] = '<a href="' . Vars::$URI . '?act=down_file&amp;id=' . Vars::$ID . '">' . __('repeat') . '</a>';
                    ...
                    if ((move_uploaded_file($_FILES["fail"]["tmp_name"], "$load_cat/$fname")) == TRUE) {
                        ...
                        echo '<div class="phdr"><a href="' . Vars::$URI. '?act=down_file&amp;id=' . Vars::$ID. '">' . __('upload_file_more') . '</a> | <a href="' . Vars::$URI. '?id=' . Vars::$ID. '">' . __('back') . '</a></div>';
                    } else
                        echo '<div class="rmenu">' . __('upload_file_no') . '<br /><a href="' . Vars::$URI. '?act=down_file&amp;id=' . Vars::$ID . '">' . __('repeat') . '</a></div>';
                }
            } else
                echo '<div class="rmenu">' . __('upload_file_no') . '<br /><a href="' . Vars::$URI. '?act=down_file&amp;id=' . Vars::$ID . '">' . __('repeat') . '</a></div>';
        } else {
        ...
        echo functions::displayError(__('access_forbidden '), '<a href="' . Vars::$URI . '?id=' . Vars::$ID . '">' . __('back') . '</a>');
} else  {
