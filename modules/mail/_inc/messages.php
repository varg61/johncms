        ...
        } else
        {
            if (Vars::$ID) {
                ...
                if (mysql_num_rows($q)) {
                    ...
                    if ($total) {
                        ...
                        while ($row = mysql_fetch_assoc($query))
                        {
                            ...
                            $array[] = array(
                                ...
                                'file'      => $row['filename'] ? Functions::getImage(Mail::fileicon($row['filename']), '', 'style="margin: 0 0 -4px 0;"') . '&#160;<a href="' . Vars::$MODULE_URI . '?act=load&amp;id=' . $row['mid'] . '">' . $row['filename'] . '</a> (' . Mail::formatsize($row['filesize']) . ')(' . $row['filecount'] . ')' : '',
                                ...
                                'selectBar' => ('[<span class="red">х</span>&#160;<a href="' . Vars::$MODULE_URI . '?act=messages&amp;mod=delete&amp;id=' . $row['mid'] . '">' . lng('delete') . '</a>] ' . (($row['elected_in'] !=
                    ...
                    if (Vars::$USER_BAN['1'] || Vars::$USER_BAN['3']) {
                        $tpl->ignor = '<div class="rmenu">' . lng('error_banned') . '</div>';
                    } else if (Mail::ignor(Vars::$ID)) {
                        $tpl->ignor = '<div class="rmenu">' . lng('user_banned') . '</div>';
                    }
            ...
            {
                $tpl->contents = Functions::displayError(lng('contact_no_select') . '!', '<a href="' .
