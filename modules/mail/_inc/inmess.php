...
WHERE `contact_id`='" . Vars::$USER_ID . "' AND `sys`='0' AND `delete_in`!='" . Vars::$USER_ID . "' AND `delete_out`!='" . Vars::$USER_ID . "'" ), 0 );
$tpl->total = $total;
if ( $total )
{
	...
	WHERE `cms_mail_messages`.`contact_id`='" . Vars::$USER_ID . "' AND `cms_mail_messages`.`sys`='0' AND `cms_mail_messages`.`delete_in`!='" . Vars::$USER_ID . "' AND `cms_mail_messages`.`delete_out`!='" . Vars::$USER_ID . "' ORDER BY `cms_mail_messages`.`time` DESC" . Vars::db_pagination() );
	$array = array();
	$i = 1;
	...
	'?act=inmes&amp;', Vars::$START, $total, Vars::
	$USER_SET['page_size'] );
}
