...
$tpl->title = Vars::$LNG['mail'] . ' | ' . $lng_mail['basket'];
if ( Vars::$ID )
{
                ...
                while ( $row = mysql_fetch_assoc( $query ) )
                {
                    ...
                    $array[] = array(
                        ...
                        'selectBar' => '[<span class="green">&laquo;</span> <a href="' . Vars::$MODULE_URI .
                            '?act=restore&amp;id=' . $row['mid'] . '">' . $lng_mail['rest'] . '</a>] [<span class="red">х</span> <a href="' .
                            Vars::$MODULE_URI . '?act=delete&amp;id=' . $row['mid'] . '">' . Vars::$LNG['delete'] .
                            '</a>]' );
                    ++$i;
                }

                $tpl->query = $array;
                $tpl->titleTest = '<div class="phdr"><h3>' . $lng_mail['deleted_message'] . '</h3></div>';
    ...
    if ( $total )
    {
        ...
        while ( $row = mysql_fetch_assoc( $query ) )
        {
            $array[] = array(
                ...
                'list' => ( ( $i % 2 ) ? 'list1' : 'list2' ),
                'nickname' => $row['nickname'],
                'count_in' => mysql_result( mysql_query( "SELECT COUNT(*) FROM `cms_messages` WHERE `user_id`='{$row['id']}' AND `delete_in`='" .
                    Vars::$USER_ID . "' AND `delete`!='" . Vars::$USER_ID . "'" ), 0 ),
                'count_out' => mysql_result( mysql_query( "SELECT COUNT(*) FROM `cms_messages` WHERE `contact_id`='{$row['id']}' AND `delete_out`='" .
                    Vars::$USER_ID . "' AND `delete`!='" . Vars::$USER_ID . "'" ), 0 ),
                'count_new' => '',
