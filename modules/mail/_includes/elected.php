    ...
    } else
    {
        ...
        if ( mysql_num_rows( $q ) )
        {
            ...
            if ( $total )
            {
				...
				AND (`cms_messages`.`elected_in`='" . Vars::$USER_ID . "'
				OR `cms_messages`.`elected_out`='" . Vars::$USER_ID . "')
                ...
                while ( $row = mysql_fetch_assoc( $query ) )
                {
                    ...
                    $array[] = array(
                        ...
                        'selectBar' => '[<span class="red">х</span> <a href="' . Vars::$MODULE_URI .
                            '?act=delete&amp;id=' . $row['mid'] . '">' . Vars::$LNG['delete'] . '</a>]' );
                    ++$i;
                }

                $tpl->query = $array;
                $tpl->titleTest = '<div class="phdr"><h3>' . $lng_mail['elected'] . '</h3></div>';
                $tpl->urlTest = '<div class="menu"><a href="' . Vars::$URI . '">' . Vars::$LNG['contacts'] .
                    '</a></div>';
                $tpl->total = $total;
            ...
            {
                Header( 'Location: ' . Vars::$MODULE_URI . '?act=elected' );
                exit;
            }
        } else
        {
...
} else
{
	...
	AND (`cms_messages`.`elected_out`='" . Vars::$USER_ID . "' 
	OR `cms_messages`.`elected_in`='" . Vars::$USER_ID . "') 
    ...
    if ( $total )
    {
        ...
        while ( $row = mysql_fetch_assoc( $query ) )
        {
            $array[] = array(
                ...
                'list' => ( ( $i % 2 ) ? 'list1' : 'list2' ),
                'nickname' => $row['nickname'],
                'count_in' => mysql_result( mysql_query( "SELECT COUNT(*) FROM `cms_messages` WHERE `user_id`='{$row['id']}' AND `elected_in`='" .
                    Vars::$USER_ID . "' AND `delete`!='" . Vars::$USER_ID . "'" ), 0 ),
                'count_out' => mysql_result( mysql_query( "SELECT COUNT(*) FROM `cms_messages` WHERE `contact_id`='{$row['id']}' AND `elected_out`='" .
                    Vars::$USER_ID . "' AND `delete`!='" . Vars::$USER_ID . "'" ), 0 ),
                'count_new' => '',
                'url' => ( Vars::$MODULE_URI . '?act=elected&amp;id=' . $row['id'] ),
                'online' => ( time() > $row['last_visit'] + 300 ? '<span class="red"> [Off]</span>' :
                    '<span class="green"> [ON]</span>' ) );
            ++$i;
        }
    ...
    {
        $tpl->contacts = '<div class="rmenu">' . $lng_mail['no_elected'] . '</div>';
    }
    $tpl->contents = $tpl->includeTpl( 'mail/elected' );
}
