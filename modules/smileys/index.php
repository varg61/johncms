...
switch (Vars::$ACT) {
        ...
        if (Vars::$USER_RIGHTS == 9) {
            ...
            if (file_put_contents(CACHEPATH . 'smileys.dat', serialize($cache))) {
                echo'<div class="gmenu">' .
                    '<p>' . __('smileys_updated') . '</p>' .
                    '<p><a href="' . Vars::$HOME_URL . '/smileys">' . __('smileys_catalog') . '</a><br/>' .
                    '<a href="' . Vars::$HOME_URL . '/admin">' . __('admin_panel') . '</a></p>' .
                    '</div>';
            } else {
                echo '<div class="rmenu"><p>' . __('smileys_error') . '</p></div>';
            }
        } else {
        ...
        if ($total) {
            if (Vars::$USER_ID && !Vars::$IS_MOBILE) {
                ...
                echo'<div class="topmenu">' .
                    '<a href="' . Vars::$URI . '?act=my_smileys">' . __('my_smileys') . '</a>  (' . count($user_sm) . ' / ' . $user_smileys . ')' .
                    '</div>' .
            ...
            for ($i = Vars::$START; $i < $end; $i++) {
                ...
                echo '<img src="' . Vars::$HOME_URL . '/assets/smileys/' . $cat . '/' . basename($smileys[$i]) . '" alt="" />&#160;:' . $smile . ': ' . __('lng_or') . ' :' . Functions::translit($smile) . ':' .
                    '</div>';
            }
            if (Vars::$USER_ID && !Vars::$IS_MOBILE) {
    ...
    case 'my_smileys':
        ...
        -----------------------------------------------------------------
        */
        echo '<div class="phdr"><a href="' . Vars::$URI . '"><b>' . __('smileys') . '</b></a> | ' . __('my_smileys') . '</div>';
        ...
        foreach ($smileys as $value) {
            ...
            echo ($i % 2 ? '<div class="list2">' : '<div class="list1">') .
                ...
                Functions::smileys($smile, Vars::$USER_RIGHTS >= 1 ? 1 : 0) . '&#160;' . $smile . ' ' . __('lng_or') . ' ' . Functions::translit($smile) . '</div>';
            $i++;
        }
        if ($total) {
            echo '<div class="rmenu"><input type="submit" name="delete" value=" ' . __('delete') . ' "/></div></form>';
        } else {
            echo '<div class="menu"><p>' . __('list_empty') . '<br /><a href="' . Vars::$URI . '">' . __('add_smileys') . '</a></p></div>';
        }
        ...
        echo '<p>' . ($total ? '<a href="' . Vars::$URI . '?act=set&amp;clean">' . __('clear') . '</a><br />'
    ...
    case 'set':
        ...
        */
        ...
        if (Vars::$IS_MOBILE || ($delete && !$_POST['delete_sm']) || ($add && !$_POST['add_sm'])) {
            echo Functions::displayError(__('error_wrong_data'), '<a href="' . Vars::$URI . '">' . __('smileys') . '</a>');
            exit;
        }
        if (($smileys = Vars::getUserData('smileys')) === FALSE) $smileys = array();
        if (!is_array($smileys))
    ...
    default:
        ...
        -----------------------------------------------------------------
        */
        echo '<div class="phdr"><a href="' . Vars::$HOME_URL . '/help"><b>F.A.Q.</b></a> | ' . __('smileys') . '</div>';

        // Меню личных смайлов
        if (Vars::$USER_ID && !Vars::$IS_MOBILE) {
            if (($smileys = Vars::getUserData('smileys')) === FALSE) $smileys = array();
            $mycount = !empty($smileys) ? count($smileys) : '0';
            echo '<div class="topmenu"><a href="' . Vars::$URI . '?act=my_smileys">' . __('my_smileys') . '</a> (' . $mycount . ' / ' . $user_smileys . ')</div>';
        }
