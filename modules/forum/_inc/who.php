...
if (Vars::$ID) {
    ...
    */
    ...
    if (mysql_num_rows($req)) {
        ...
        echo '<div class="phdr"><b>' . __('who_in_topic') . ':</b> <a href="' . $url . '?id=' . Vars::$ID . '">' . $res['text'] . '</a></div>';
        if (Vars::$USER_RIGHTS > 0)
            echo '<div class="topmenu">' . ($do == 'guest' ? '<a href="' . $url . '?act=who&amp;id=' . Vars::$ID . '">' . __('authorized') . '</a> | ' . __('guests')
                    : __('authorized') . ' | <a href="' . $url . '?act=who&amp;do=guest&amp;id=' . Vars::$ID . '">' . __('guests') . '</a>') . '</div>';
    ...
    echo '<div class="phdr">' . __('total') . ': ' . $total . '</div>' .
         '<p><a href="' . $url . '?id=' . Vars::$ID . '">' . __('to_topic') . '</a></p>';
} else {
    /*
    -----------------------------------------------------------------
    ...
    */
    ...
    if ($total > Vars::$USER_SET['page_size']) echo '<div class="topmenu">' . Functions::displayPagination($url . '?act=who&amp;' . ($do == 'guest' ? 'do=guest&amp;' : ''), Vars::$START, $total, Vars::$USER_SET['page_size']) . '</div>';
    if ($total) {
    ...
    if ($total > Vars::$USER_SET['page_size']) {
        echo '<div class="topmenu">' . Functions::displayPagination($url . '?act=who&amp;' . ($do == 'guest' ? 'do=guest&amp;' : ''), Vars::$START, $total, Vars::$USER_SET['page_size']) . '</div>' .
