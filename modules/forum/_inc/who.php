...
if (Vars::$ID) {
    ...
    */
    ...
    if (mysql_num_rows($req)) {
        ...
        echo '<div class="phdr"><b>' . lng('who_in_topic') . ':</b> <a href="index.php?id=' . Vars::$ID . '">' . $res['text'] . '</a></div>';
        if (Vars::$USER_RIGHTS > 0)
            echo '<div class="topmenu">' . ($do == 'guest' ? '<a href="index.php?act=who&amp;id=' . Vars::$ID . '">' . lng('authorized') . '</a> | ' . lng('guests')
                    : lng('authorized') . ' | <a href="index.php?act=who&amp;do=guest&amp;id=' . Vars::$ID . '">' . lng('guests') . '</a>') . '</div>';
    ...
    echo '<div class="phdr">' . lng('total') . ': ' . $total . '</div>' .
         '<p><a href="index.php?id=' . Vars::$ID . '">' . lng('to_topic') . '</a></p>';
} else {
    /*
    -----------------------------------------------------------------
    ...
    -----------------------------------------------------------------
    */
    echo '<div class="phdr"><a href="index.php"><b>' . lng('forum') . '</b></a> | ' . lng('who_in_forum') . '</div>';
    if (Vars::$USER_RIGHTS > 0)
    ...
    if ($total) {
        ...
        while (($res = mysql_fetch_assoc($req)) !== false) {
            ...
            switch ($res['place']) {
                case 'forum':
                    $place = '<a href="index.php">' . lng('place_main') . '</a>';
                    break;
                ...
                case 'forumfiles':
                    $place = '<a href="index.php?act=files">' . lng('place_files') . '</a>';
                    break;

                case 'forumnew':
                    $place = '<a href="index.php?act=new">' . lng('place_new') . '</a>';
                    break;
                            ...
                            switch ($res_t['type']) {
                                ...
                                case 'm':
                                    ...
                                    if (mysql_num_rows($req_m)) {
                                        ...
                                        $place = (isset($where[2]) ? lng('place_answer') : lng('place_topic')) . ' &quot;<a href="index.php?id=' . $res_t['refid'] . '">' . $res_m['text'] . '</a>&quot;';
                                    }
                                    break;
                            }
                        }
