<ul class="nav">
    <li><h1<?= $this->user['id'] == Vars::$USER_ID ? ' class="section-personal"' : '' ?>><?= __('system_settings') ?></h1></li>
</ul>
...
<div class="form-container">
    ...
    <form action="<?= Router::getUrl(3) ?>?act=edit_settings" method="post">
        <div class="form-block">
            ...
            <label class="small"><input name="translit" type="checkbox" value="1" <?= (Vars::$USER_SET['translit'] ? 'checked="checked"' : '') ?>/>&#160;<?= __('translit') ?></label><br/>
            <?php endif; ?>
            <label class="small"><input name="avatar" type="checkbox" value="1" <?= (Vars::$USER_SET['avatar'] ? 'checked="checked"' : '') ?>/>&#160;<?= __('avatars') ?></label><br/>
            <label class="small"><input name="smileys" type="checkbox" value="1" <?= (Vars::$USER_SET['smileys'] ? 'checked="checked"' : '') ?>/>&#160;<?= __('smileys') ?></label><br/>

            <label for="skin"><?= __('design_template') ?></label><br/>
            ...
            <label><?= __('language_select') ?></label><br/>
