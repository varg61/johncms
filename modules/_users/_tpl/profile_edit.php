...
<div class="form-container">
    ...
    <form name="form" action="<?= Vars::$URI ?>?act=edit&amp;user=<?= $this->user['id'] ?>" method="post">
        <div class="form-block">
            <label for="imname"><?= __('name') ?></label><br/>
            <input id="imname" type="text" value="<?= htmlspecialchars($this->user['imname']) ?>" name="imname"/><br/>
            <span class="input-help"><?= __('description_name') ?></span><br/>

            <?php if (Vars::$USER_SYS['change_sex'] || Vars::$USER_RIGHTS >= 7) : ?>
            <label><?= __('sex') ?></label><br/>
            <label class="small"><input id="sex" type="radio" value="m" name="sex" <?= ($this->user['sex'] == 'm' ? 'checked="checked"' : '') ?>/>&#160;<?= __('sex_m') ?></label><br/>
            <label class="small"><input type="radio" value="w" name="sex" <?= ($this->user['sex'] == 'w' ? 'checked="checked"' : '') ?>/>&#160;<?= __('sex_w') ?></label><br/>
            <?php endif; ?>

            <label for="birth"><?= __('birthday') ?></label><br/>
            ...
            <span class="input-help"><?= __('description_birth') ?></span><br/>

            <label for="live"><?= __('live') ?></label><br/>
            <input id="live" type="text" value="<?= htmlspecialchars($this->user['live']) ?>" name="live"/><br/>
            <span class="input-help"><?= __('description_live') ?></span><br/>
            ...
            <span class="input-help"><?= __('description_about') ?></span><br/>

            <label for="tel"><?= __('phone_number') ?></label><br/>
            <input id="tel" type="text" value="<?= htmlspecialchars($this->user['tel']) ?>" name="tel"/><br/>
            <span class="input-help"><?= __('description_phone_number') ?></span><br/>
            ...
            <br/>
            <label class="small"><input name="mailvis" type="checkbox" value="1"<?= ($this->user['mailvis'] ? ' checked="checked"' : '') ?>/>&#160;<?= __('show_in_profile') ?></label><br/>
            <span class="input-help"><?= __('description_email') ?></span><br/>

            <label for="siteurl"><?= __('site') ?></label><br/>
            <input id="siteurl" type="text" value="<?= htmlspecialchars($this->user['siteurl']) ?>" name="siteurl"/><br/>
            <span class="input-help"><?= __('description_siteurl') ?></span><br/>

            <label for="skype">Skype</label><br/>
            <input id="skype" type="text" value="<?= htmlspecialchars($this->user['skype']) ?>" name="skype"/><br/>
            <span class="input-help"><?= __('description_skype') ?></span><br/>

            <label for="icq">ICQ</label><br/>
            <input id="icq" type="text" value="<?= $this->user['icq'] ?>" name="icq" size="10" maxlength="10"/><br/>
            <span class="input-help"><?= __('description_icq') ?></span><br/><br/>
