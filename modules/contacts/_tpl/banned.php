...
<? if ($this->total): ?><form action="<?= Router::getUri(2) ?>?act=banned" method="post"><div><? endif ?>
<?= $this->contacts ?>
<? if ($this->total): ?>
    <div class="gmenu">
        <?= __('noted_contacts') ?>:<br/>
        <input type="hidden" name="token" value="<?= $this->token ?>"/>
        <input type="submit" name="unban" value="<?= __('unban') ?>"/> <input type="submit" name="delete" value="<?= __('delete') ?>"/><br/>
    </div>
	</div></form>
