...
if (Vars::$USER_RIGHTS >= 9) {
    ...
    <div class="sub"><a href="?act=ip_base&amp;action=base_edit&amp;id=' . $arr['id'] . '">' . __('edit') . '</a> | <a href="?act=ip_base&amp;action=base_delete&amp;id=' . $arr['id'] . '">' . __('delete') . '</a></div></div>';
                ...
                if ($count_ip > Vars::$USER_SET['page_size']) {
                    ...
                    echo Functions::displayPagination($url . '?act=ip_base&amp;action=base&amp;', Vars::$START, $count_ip, Vars::$USER_SET['page_size']) . '</div>';
                    echo '<p><form action="' . $url . '" method="get"><input type="hidden" name="act" value="ip_base"/><input type="hidden" name="action" value="base"/><input type="text" name="page" size="2"/><input type="submit" value="' . __('to_page') . ' &gt;&gt;"/></form></p>';
                }

            } else {
                echo'<div class="rmenu">' . __('no_data') . '!</div>';
            }
            echo'<div class="menu"><a href="' . $url . '?act=ip_base&amp;action=base_add">' . __('add_ip') . '</a></div>';
            break;
