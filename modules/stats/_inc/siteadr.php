...
if (!$site) {
    ...
    if ($count > 0) {
        ...
        if ($count > Vars::$USER_SET['page_size']) {
            ...
            echo Functions::displayPagination($url . '?act=siteadr&amp;site=' . $site . '&amp;', Vars::$START, $count, Vars::$USER_SET['page_size']) . '</div>';
            echo '<p><form action="' . $url . '" method="get"><input type="hidden" name="act" value="siteadr"/><input type="hidden" name="site" value="' . $site . '"/><input type="text" name="page" size="2"/><input type="submit" value="' . __('to_page') . ' &gt;&gt;"/></form></p>';
        }
