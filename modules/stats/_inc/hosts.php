...
echo '<div class="phdr">'.lng('view_hosts').'</div>';
...
if($count > 0){
    ...
    echo '<div class="phdr">'.lng('total_hosts').': '.$count.'</div>';
    if ($count > Vars::$USER_SET['page_size']){
    	...
    	echo '<p><form action="'.Vars::$URI.'" method="get"><input type="hidden" name="act" value="hosts"/><input type="text" name="page" size="2"/><input type="submit" value="' . lng('to_page') . ' &gt;&gt;"/></form></p>';}
}else{
