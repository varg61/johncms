...
echo '<div class="phdr">'.lng('popular_searches').'</div>';
...
if($count > 0){
    ...
    while($arr = mysql_fetch_array($req)){
        ...
        if($arr['pop'] !== '/'){
        echo '<b>'.Functions::displayDate($arr['date']).'</b> | '.lng('title').': '.($arr['head'] == '' ? lng('no_title') : $arr['head']);
    ...
    if ($count > Vars::$USER_SET['page_size']){
    	...
    	echo Functions::displayPagination(Vars::$URI.'?act=pop&amp;', Vars::$START, $count, Vars::$USER_SET['page_size']) . '</div>';
    	echo '<p><form action="'.Vars::$URI.'" method="get"><input type="hidden" name="act" value="pop"/><input type="text" name="page" size="2"/><input type="submit" value="'.lng('to_page').' &gt;&gt;"/></form></p>';}
