...
echo '<div class="phdr">'.lng('countries').' ('.lng('hits').')</div>';
...
if($count > 0){
    ...
    if ($count > Vars::$USER_SET['page_size']){
    	...
    	echo '<p><form action="'.Vars::$URI.'" method="get"><input type="hidden" name="act" value="country"/><input type="text" name="page" size="2"/><input type="submit" value="'.lng('to_page').' &gt;&gt;"/></form></p>';}
