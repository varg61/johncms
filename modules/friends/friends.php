...
if ($id && $id != $user_id && $do) {
		...
		case 'add':
			...
			if($fr != 2) {
				if(isset($_POST['submit'])) {
					...
					$friends = isset($_POST['friends']) && $_POST['friends'] >= 0 && $_POST['friends'] <= 6 ? abs(intval($_POST['friends'])) : 0;
					$arr_fr = array(
					...
					<input type="radio" value="6" name="friends" />&#160;' . $lng_profile['relative'] . '<br />
		...
		case 'ok':
			...
			if($fr != 2) {
				if(isset($_POST['submit'])) {
					...
					mysql_query("DELETE FROM `cms_mail` WHERE `user_id` = '$id' AND `from_id` = '$user_id' AND `text`='$text'");
					echo '<div class="rmenu">' . $lng_profile['confirmed_friendship'] . ' ' . $result['name'] . ' ' . $lng_profile['your_friend'] . '.</div>';
				} else {
	...
	if($user['id'] && $user['id'] != $user_id) {
		...
		switch($do) {
			case 'demands':
				echo '<div class="phdr"><h3>' . $lng_profile['friends'] . '</h3></div>';
				echo '<div class="topmenu">' . $lng_profile['my_demand'] . '</div>';
				...
				if ($total) {
					...
					echo '<div class="rmenu">' . $lng_profile['not_demands'] . '</div>';
				}
				echo '<div class="bmenu"><a href="profile.php?act=friends">' . $lng['back'] . '</a></div>';
			break;
			
			case 'offers':
				echo '<div class="phdr"><h3>' . $lng_profile['friends'] . '</h3></div>';
				echo '<div class="topmenu">' . $lng_profile['my_offers'] . '</div>';
				...
				if ($total) {
					...
					echo '<div class="rmenu">' . $lng_profile['not_offers'] . '</div>';
				}
				echo '<div class="bmenu"><a href="profile.php?act=friends">' . $lng['back'] . '</a></div>';
			break;
			...
			case 'settings':
				$set_mail = unserialize($datauser['set_mail']);
				if(isset($_POST['submit'])) {
					$set_mail['access'] = isset($_POST['access']) && $_POST['access'] >=0 && $_POST['access'] <= 2 ? abs(intval($_POST['access'])) : 0;
					mysql_query("UPDATE `users` SET `set_mail` = '" . mysql_real_escape_string(serialize($set_mail)) . "' WHERE `id` = '$user_id'");
				}
				...
				<input type="radio" value="0" name="access" ' . (!$set_mail['access'] ? 'checked="checked"' : '') . '/>&#160;' . $lng_profile['write_all'] . '<br />
				<input type="radio" value="1" name="access" ' . ($set_mail['access'] == 1 ? 'checked="checked"' : '') . '/>&#160;' . $lng_profile['write_contacts'] . '<br />
				<input type="radio" value="2" name="access" ' . ($set_mail['access'] == 2 ? 'checked="checked"' : '') . '/>&#160;' . $lng_profile['write_friends'] . '</div>
			...
			echo '<div class="topmenu"><a href="profile.php?act=friends&amp;do=demands">' . $lng_profile['my_demand'] . '</a> ' . ($dem ? '(<span class="red">' . $dem . '</span>)' : '') . '| <a href="profile.php?act=friends&amp;do=offers">' . $lng_profile['my_offers'] . '</a> ' . ($off ? '(<span class="red">' . $off . '</span>)' : '') . '</div>';
			$sort = isset($_REQUEST['sort']) && $_REQUEST['sort'] >=0 && $_REQUEST['sort'] <= 6 ? abs(intval($_REQUEST['sort'])) : 0;
			if($sort) {
				$sql = " AND `cms_contact`.`man`='$sort'";
				$nav = '&amp;sort=' . $sort;
			...
			$total = mysql_result(mysql_query("SELECT COUNT(*) FROM `cms_contact`
			LEFT JOIN `users` ON `cms_contact`.`from_id`=`users`.`id`
			WHERE `cms_contact`.`user_id`='" . $user_id . "' AND `cms_contact`.`type`='2' AND `cms_contact`.`friends`='1' AND `cms_contact`.`ban`!='1'$sql
			"), 0);
			if ($total) {
				$req = mysql_query("SELECT `users`.* FROM `cms_contact`
				LEFT JOIN `users` ON `cms_contact`.`from_id`=`users`.`id`
				WHERE `cms_contact`.`user_id`='" . $user_id . "' AND `cms_contact`.`type`='2' AND `cms_contact`.`friends`='1' AND `cms_contact`.`ban`!='1'$sql ORDER BY `cms_contact`.`time` DESC LIMIT " . $start . "," . $kmess
				);
				for ($i = 0; ($row = mysql_fetch_assoc($req)) !== false; ++$i) {
					echo $i % 2 ? '<div class="list1">' : '<div class="list2">';
					$subtext = '<a href="../mail/index.php?act=write&amp;id=' . $row['id'] . '">' . $lng['write'] . '</a> | <a href="profile.php?act=friends&amp;do=delete&amp;id=' . $row['id'] . '">' . $lng['delete'] . '</a> | <a href="../mail/index.php?act=ignor&amp;id=' . $row['id'] . '&amp;add">' . $lng_profile['add_ignor'] . '</a>';
