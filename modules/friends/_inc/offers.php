...
WHERE `cms_mail_contacts`.`contact_id`='" . Vars::$USER_ID . "' AND `cms_mail_contacts`.`access`='2' AND `cms_mail_contacts`.`friends`='0' AND `cms_mail_contacts`.`banned`!='1'
"), 0);
if ($tpl->total) {
	...
	$tpl->query = $array;
	$tpl->display_pagination = Functions::displayPagination( Vars::$MODULE_URI . '?act=demands&amp;', Vars::$START, $total, Vars::$USER_SET['page_size'] );
}
