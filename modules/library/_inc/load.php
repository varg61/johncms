...
if (Vars::$USER_RIGHTS == 5 || Vars::$USER_RIGHTS >= 6) {
    ...
    if ($ms['ip'] == 0) {
        if (isset($_POST['submit'])) {
            if (empty($_POST['name'])) {
                echo Functions::displayError(lng('error_empty_title'), '<a href="' . Vars::$URI . '?act=load&amp;id=' . Vars::$ID . '">' . lng('repeat') . '</a>');
                exit;
            }
            ...
            if ($fname != "") {
                if (eregi("[^a-z0-9.()+_-]", $fname)) {
                    echo "Invalid file name<br /><a href='" . Vars::$URI . "?act=load&amp;id=" . Vars::$ID . "'>" . lng('repeat') . "</a><br/>";
                    exit;
                }
                if ((preg_match("/.php/i", $fname)) or (preg_match("/.pl/i", $fname)) or ($fname == ".htaccess")) {
                    echo "Invalid file format<br/><a href='" . Vars::$URI . "?act=load&amp;id=" . Vars::$ID . "'>" . lng('repeat') . "</a><br/>";
                    exit;
                }
                if ($ftip != "txt") {
                    echo "This is not a text file<br/><a href='" . Vars::$URI . "?act=load&amp;id=" . Vars::$ID . "'>" . lng('repeat') . "</a><br/>";
                    exit;
                }
                if ((move_uploaded_file($_FILES["fail"]["tmp_name"], "temp/$fname")) == true) {
                    ...
                    if (mb_check_encoding($txt, 'UTF-8')) {
                        ...
                        echo "File in an unknown encoding<br /><a href='" . Vars::$URI . "?act=load&amp;id=" . Vars::$ID . "'>" . lng('repeat') . "</a><br/>";
                        exit;
                    }
                    ...
                    unlink("temp/$ch");
                    $cid = mysql_insert_id();
                    echo lng('article_added') . "<br/><a href='" . Vars::$URI . "?id=" . $cid . "'>" . lng('to_article') . "</a><br/>";
                } else {
                    echo lng('error_uploading') . "<br/><a href='" . Vars::$URI . "?act=load&amp;id=" . Vars::$ID . "'>" . lng('repeat') . "</a><br/>";
                    exit;
                }
            }
        } else {
            echo '<h3>' . lng('upload_article') . '</h3>' . lng('supported_encoding') . ' Win-1251, KOI8-R, UTF-8<br/><br/>' .
                 '<form action="' . Vars::$URI . '?act=load&amp;id=' . Vars::$ID . '" method="post" enctype="multipart/form-data">' .
