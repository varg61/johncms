...
if (isset($actions[Vars::$ACT])
    ...
    if (!Vars::$ID) {
        ...
        if ($res > 0)
            echo '<a href="index.php?act=new">' . lng('new_articles') . '</a> (' . $res . ')<br/>';
        echo '<a href="index.php?act=topread">' . lng('most_readed') . '</a></p></div>';
        Vars::$ID = 0;
        $tip = "cat";
    } else {
        $tip = $zag['type'];
        ...
        case 'cat':
            ...
            if ($total > Vars::$USER_SET['page_size']) {
                echo '<div class="topmenu">' . Functions::displayPagination('index.php?id=' . Vars::$ID . '&amp;', Vars::$START, $total, Vars::$USER_SET['page_size']) . '</div>' .
                    '<p><form action="index.php" method="get"><input type="hidden" name="id" value="' . Vars::$ID . '"/><input type="text" name="page" size="2"/><input type="submit" value="' . lng('to_page') . ' &gt;&gt;"/></form></p>';
            }
            echo '<p>';
            if ((Vars::$USER_RIGHTS == 5 || Vars::$USER_RIGHTS >= 6) && Vars::$ID != 0) {
                ...
                if ($ct1 == 0) {
                    echo '<a href="index.php?act=del&amp;id=' . Vars::$ID . '">' . lng('delete_category') . '</a><br/>';
                }
                echo '<a href="index.php?act=edit&amp;id=' . Vars::$ID . '">' . lng('edit_category') . '</a><br/>';
            }
            if ((Vars::$USER_RIGHTS == 5 || Vars::$USER_RIGHTS >= 6) && (isset($zag['ip']) && $zag['ip'] == 1 || Vars::$ID == 0)) {
                echo '<a href="index.php?act=mkcat&amp;id=' . Vars::$ID . '">' . lng('create_category') . '</a><br/>';
            }
            if (isset($zag['ip']) && $zag['ip'] == 0 && Vars::$ID != 0) {
                if ((Vars::$USER_RIGHTS == 5 || Vars::$USER_RIGHTS >= 6) || ($zag['soft'] == 1 && !empty($_SESSION['uid']))) {
                    echo "<a href='index.php?act=write&amp;id=" . Vars::$ID . "'>" . lng('write_article') . "</a><br/>";
                }
                if (Vars::$USER_RIGHTS == 5 || Vars::$USER_RIGHTS >= 6) {
                    echo "<a href='index.php?act=load&amp;id=" . Vars::$ID . "'>" . lng('upload_article') . "</a><br/>";
                }
            }
            if (Vars::$ID) {
        ...
        case 'bk':
            ...
            */
            ...
            if (Vars::$SYSTEM_SET['mod_lib_comm'] || Vars::$USER_RIGHTS >= 7) {
                ...
                $comm_link = "<a href='index.php?act=komm&amp;id=" . Vars::$ID . "'>" . lng('comments') . "</a> ($km1)";
            } else {
                $comm_link = '&#160;';
            }
            ...
            if (Vars::$USER_RIGHTS == 5 || Vars::$USER_RIGHTS >= 6) {
                echo '<p><a href="index.php?act=edit&amp;id=' . Vars::$ID . '">' . lng('edit') . '</a><br/>';
                echo '<a href="index.php?act=del&amp;id=' . Vars::$ID . '">' . lng('delete') . '</a></p>';
            }
            echo '<a href="index.php?act=java&amp;id=' . Vars::$ID . '">' . lng('download_java') . '</a><br /><br />';
            ...
            echo "<a href='index.php?'>" . lng('to_library') . "</a>";
            break;
        default :
