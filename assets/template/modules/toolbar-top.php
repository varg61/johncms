...
<table id="nav-top">
    <tr>
        ...
        <td>
            <a href="<?= Vars::$HOME_URL ?>/users/<?= Vars::$USER_ID ?>/assets/" <?= (Vars::$PLACE == 'users/profile?act=assets' ? 'class="select"' : '') ?> title="<?= __('personal') ?>"><?= Functions::loadImage('toolbar-account.png', 44, '', __('personal')) ?></a>
        </td>
        <?php else: ?>
