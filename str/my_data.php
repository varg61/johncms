...
if ($id && $rights >= 7 && $act == 'reset') {
    ...
    */
    ...
    echo '<div class="gmenu"><p>' . $lng['settings_default'] . '<br /><a href="anketa.php?id=' . $user['id'] . '">' . $lng['to_form'] . '</a></p></div>';
    require('../incfiles/end.php');
    exit;
}
echo '<div class="phdr"><a href="anketa.php?id=' . $user['id'] . '"><b>' . ($id && $id != $user_id ? $lng['profile'] : $lng_profile['my_profile']) . '</b></a> | ' . $lng['edit'] . '</div>';
...
echo '<form action="my_data.php?id=' . $user['id'] . '" method="post">' .
    ...
    '<li>Логин: <b>' . $user['name_lat'] . '</b></li>';
if ($rights >= 7) {
    echo '<li>' . $lng['nick'] . ': (' . $lng_profile['nick_lenght'] . ')<br /><input type="text" value="' . $user['name'] . '" name="name" /></li>' .
        '<li>' . $lng['status'] . ': (' . $lng_profile['status_lenght'] . ')<br /><input type="text" value="' . $user['status'] . '" name="status" /></li>' .
        '<li><a href="my_pass.php?id=' . $id . '">' . $lng['change_password'] . '</a></li>';
} else {
    echo '<li><span class="gray">' . $lng['nick'] . ':</span> <b>' . $user['name'] . '</b></li>' .
        '<li><span class="gray">' . $lng['status'] . ':</span> ' . $user['status'] . '</li>';
}
echo '<li>' . $lng['avatar'] . ':<br />';
$link = '';
...
echo '<small><a href="my_images.php?act=avatar&amp;id=' . $user['id'] . '">' . $lng_profile['upload'] . '</a> | ' .
    '<a href="avatar.php?id=' . $user['id'] . '">' . $lng['select'] . '</a>' . $link . '</small></li>' .
    '<li>' . $lng_profile['photo'] . ':<br />';
$link = '';
...
echo '<small><a href="my_images.php?act=up_photo&amp;id=' . $user['id'] . '">' . $lng_profile['upload'] . '</a>' . $link . '</small></li>' .
    '</ul></p></div>' .
    '<div class="menu"><p><h3><img src="../images/contacts.png" width="16" height="16" class="left" />&#160;' . $lng_profile['personal_data'] . '</h3><ul>' .
    '<li><span class="gray">' . $lng_profile['name'] . ':</span><br /><input type="text" value="' . $user['imname'] . '" name="imname" /></li>' .
    '<li><span class="gray">' . $lng_profile['birth_date'] . '</span><br />' .
    ...
    '<input type="text" value="' . $user['yearofbirth'] . '" size="4" maxlength="4" name="yearofbirth" /></li>' .
    '<li><span class="gray">' . $lng_profile['city'] . ':</span><br /><input type="text" value="' . $user['live'] . '" name="live" /></li>' .
    '<li><span class="gray">' . $lng_profile['about'] . ':</span><br /><textarea cols="' . $set_user['field_w'] . '" rows="' . $set_user['field_h'] . '" name="about">' . str_replace('<br />', "\r\n", $user['about']) . '</textarea></li>' .
    '</ul></p>' .
    '<p><h3><img src="../images/mail.png" width="16" height="16" class="left" />&#160;' . $lng_profile['communication'] . '</h3><ul>' .
    '<li><span class="gray">' . $lng_profile['phone_number'] . ':</span><br /><input type="text" value="' . $user['mibile'] . '" name="mibile" /></li>' .
    '<li><span class="gray">E-mail:</span><br /><small>' . $lng_profile['email_warning'] . '</small><br />' .
    '<input type="text" value="' . $user['mail'] . '" name="mail" /><br />' .
    '<input name="mailvis" type="checkbox" value="1" ' . ($user['mailvis'] ? 'checked="checked"' : '') . ' />&#160;' . $lng_profile['show_in_profile'] . '</li>' .
    '<li><span class="gray">ICQ:</span><br /><input type="text" value="' . $user['icq'] . '" name="icq" size="10" maxlength="10" /></li>' .
    '<li><span class="gray">Skype:</span><br /><input type="text" value="' . $user['skype'] . '" name="skype" /></li>' .
    '<li><span class="gray">Jabber:</span><br /><input type="text" value="' . $user['jabber'] . '" name="jabber" /></li>' .
    '<li><span class="gray">' . $lng_profile['site'] . ':</span><br /><input type="text" value="' . $user['www'] . '" name="www" /></li>' .
    '</ul></p></div>';
...
if ($rights >= 7) {
    ...
    echo '<li><a href="my_pass.php?id=' . $user['id'] . '">' . $lng['change_password'] . '</a></li>' .
...
echo '<div class="gmenu"><input type="submit" value="' . $lng['save'] . '" name="submit" /></div>' .
    '</form>' .
    '<div class="phdr"><a href="anketa.php' . ($id ? '?id=' . $id : '') . '">' . $lng['to_form'] . '</a></div>';
