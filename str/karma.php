...
if ($set_karma['on'] && $user_id) {
    switch ($act) {
        case 'user':
            ...
            if (!$error) {
                if ($datauser['total_on_site'] < $set_karma['karma_time'] || $datauser['postforum'] < $set_karma['forum'])
                    $error[] = 'В голосовании могут принимать участие только пользователи, пробывшие на сайте не менее ' . ($set_karma['time'] ? ($set_karma['karma_time'] / 3600) . ' час.' : ($set_karma['karma_time'] / 86400) . ' дн.') . ' и набравших на форуме ' . $set_karma['forum'] . ' пост.';
                ...
                if (($set_karma['karma_points'] - $sum) <= 0)
                    $error[] = 'Лимит голосов на сегодня исчерпан. Голоса будут начислены ' . date('d.m.y в H:i:s', ($datauser['karma_time'] + 86400));
            }
            if ($error) {
                ...
                if (isset($_POST['submit'])) {
                    ...
                    echo '<div class="phdr"><b>Отдамаем голос за ' . $res['name'] . '</b></div><form action="karma.php?act=user&amp;id=' . $id . '" method="post"><div class="gmenu">
            <b>Тип голоса:</b><br /><input name="type" type="radio" value="1" checked="checked"/> Положительный<br /><input name="type" type="radio" value="0"/> Отрицательный<br />
            <b>Колличество голосов:</b><br /><select size="1" name="points">';
                    for ($i = 1; $i < ($set_karma['karma_points'] - $sum + 1); $i++) {
                        echo '<option value="' . $i . '">' . $i . '</option>';
                    }
                    echo '</select><b><br />Комментарий:</b><br /><input name="text" type="text" value=""/><br /><small>максимум 500 символов</small><p><input type="submit" name="submit" value="Голосовать"/></p></div></form><div class="list2"><a href="anketa.php?id=' . $id . '">Анкета пользователя</a></div>';
                }
            }
            break;
        case 'new':
            ...
            if ($total) {
                $req = mysql_query("SELECT `karma_users`.* FROM `karma_users` WHERE `karma_user`='$user_id' AND `time` > " . ($realtime - 86400) . " ORDER BY `time` DESC LIMIT $start, $kmess");
            ...
            if ($total > $kmess) {
                echo '<p>' . pagenav('karma.php?act=new&amp', $start, $total, $kmess) . '</p>';
        ...
        default:
            ...
            echo '<div class="phdr"><b>Карма ' . $user['name'] . '</b> ' . $user['karma'] . ' (<span class="green">' . $exp[0] . '</span>/<span class="red">' . $exp[1] . '</span>)</div><div class="gmenu">';
            $type = abs(intval($_GET['type']));
            $sql = '';
            switch ($type) {
                case 2:
                    echo '<a href="karma.php?id=' . $id . '&amp;type=0">Все</a> | <a href="karma.php?id=' . $id . '&amp;type=1">Положительные</a> | <b>Отрицательные</b>';
                    $sql = ' AND `type` = 0';
                    break;
                case 1:
                    echo '<a href="karma.php?id=' . $id . '&amp;type=0">Все</a> | <b>Положительные</b> | <a href="karma.php?id=' . $id . '&amp;type=2">Отрицательные</a>';
                    $sql = ' AND `type` = 1';
                    break;
                default:
                    echo '<b>Все</b> | <a href="karma.php?id=' . $id . '&amp;type=1">Положительные</a> | <a href="karma.php?id=' . $id . '&amp;type=2">Отрицательные</a>';
                    $type = 0;
            }
            echo '</div>';
            $total = mysql_result(mysql_query("SELECT COUNT(*) FROM `karma_users` WHERE `karma_user`='$id' $sql"), 0);
            if ($total) {
                $req = mysql_query("SELECT `karma_users`.* FROM `karma_users` WHERE `karma_user`='$id' $sql ORDER BY `time` DESC LIMIT $start, $kmess");
                while ($res = mysql_fetch_assoc($req)) {
                    echo ($i % 2) ? '<div class="list2">' : '<div class="list1">';
                    echo $res['type'] ? '<span class="green">+' . $res['points'] . '</span> ' : '<span class="red">-' . $res['points'] . '</span> ';
                    echo $user_id == $res['user_id'] || !$res['user_id'] ? '<b>' . $res['name'] . '</b>' : '<a href="anketa.php?id=' . $res['user_id'] . '"><b>' . $res['name'] . '</b></a>';
            ...
            if ($total > $kmess) {
                echo '<p>' . pagenav('karma.php?id=' . $id . '&amp;type=' . $type . '&amp', $start, $total, $kmess) . '</p>';
                echo '<p><form action="karma.php" method="get"><input type="hidden" name="id" value="' . $id . '"/><input type="hidden" name="type" value="' . $type . '"/><input type="text" name="page" size="2"/><input type="submit" value="К странице &gt;&gt;"/></form></p>';
            }
            echo '<div class="list2"><a href="anketa.php?id=' . $id . '">Анкета пользователя</a></div>';
    }
}
require_once ('../incfiles/end.php');
