        ...
        if (file_exists($file) && filemtime($file) > (time() - $this->cache_forum_contents * 3600)) {
            ...
            if (mysql_num_rows($req)) {
                ...
                if (mysql_num_rows($req_t)) {
                    while (($res_t = mysql_fetch_assoc($req_t)) !== false) $row[] = '<a href="' . Vars::$SYSTEM_SET['homeurl'] . '/forum/index.php?id=' . $res_t['id'] . '">' . $res_t['text'] . '</a>';
        ...
        if (file_exists($file) && filemtime($file) > (time() - $this->cache_lib_contents * 3600)) {
            ...
            if (mysql_num_rows($req)) {
                ...
                if (mysql_num_rows($req_a)) {
                    while (($res_a = mysql_fetch_assoc($req_a)) !== false) $row[] = '<a href="' . Vars::$SYSTEM_SET['homeurl'] . '/library/index.php?id=' . $res_a['id'] . '">' . Validate::filterString($res_a['name']) . '</a>';
            ...
            if (mysql_num_rows($req)) {
                while (($res = mysql_fetch_assoc($req)) !== false) {
                    ...
                    if ($count) {
                        ...
                        if ($pages > 1) {
                            for ($i = 0; $i < $pages; $i++) {
                                $out[] = '<a href="' . Vars::$SYSTEM_SET['homeurl'] . '/forum/contents.php?id=' . $res['id'] . '&amp;p=' . $i . '">' . Validate::filterString($text) . ' (' . ($i + 1) . ')</a>';
                            }
                        } else {
                            $out[] = '<a href="' . Vars::$SYSTEM_SET['homeurl'] . '/forum/contents.php?id=' . $res['id'] . '">' . Validate::filterString($text) . '</a>';
                        }
                    }
                }
                if (isset($out))
            ...
            if (mysql_num_rows($req)) {
                while (($res = mysql_fetch_assoc($req)) !== false) {
                    ...
                    if ($count) {
                        ...
                        if ($pages > 1) {
                            for ($i = 0; $i < $pages; $i++) {
                                $out[] = '<a href="' . Vars::$SYSTEM_SET['homeurl'] . '/library/contents.php?id=' . $res['id'] . '&amp;p=' . $i . '">' . Validate::filterString($text) . ' (' . ($i + 1) . ')</a>';
                            }
                        } else {
