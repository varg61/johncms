<div class="phdr">
    <b><?= Vars::$LNG['information'] ?></b>
</div>
<?= $mp->news ?>
<div class="menu">
    <a href="news/index.php"><?= Vars::$LNG['news_archive'] ?></a>(<?= $mp->newscount ?>)
</div>
<div class="menu">
    <a href="pages/faq.php"><?= Vars::$LNG['information'] ?>, FAQ</a>
</div>
<div class="phdr">
    <b><?= Vars::$LNG['dialogue'] ?></b>
</div>
<!--// Ссылка на Гостевую-->
<?php if (Vars::$SYSTEM_SET['mod_guest'] || Vars::$USER_RIGHTS >= 7) { ?>
<div class="menu">
    <a href="guestbook/"><?= Vars::$LNG['guestbook'] ?></a>(<?= $count->guestbook . (Vars::$USER_RIGHTS ? '&#160;/&#160;<span class="red">' . $count->adminclub . '</span>' : '') ?>)
</div>
<?php } ?>
<!--// Ссылка на Форум-->
...
<div class="menu">
    <a href="gallery/"><?=Vars::$LNG['gallery']?></a>(<?= $count->gallery . ($count->gallery_new ? '&#160;/&#160;<span class="red">+' . $count->gallery_new . '</span>' : '') ?>)
</div>
<?php } ?>
<!--// Ссылки на пользователей и фотоальбомы-->
<?php if (Vars::$USER_ID || Vars::$SYSTEM_SET['active']) { ?>
<div class="phdr">
    <b><?= Vars::$LNG['community'] ?></b>
</div>
<div class="menu">
    <a href="users/index.php"><?=Vars::$LNG['users']?></a>(<?= $count->users . ($count->users_new ? '&#160;/&#160;<span class="red">+' . $count->users_new . '</span>' : '') ?>)
</div>
<div class="menu">
    <a href="users/album.php"><?=Vars::$LNG['photo_albums']?></a>(<?= $count->album . '&#160;/&#160;' . $count->album_photo . ($count->album_photo_new ? '&#160;/&#160;<span class="red">+' . $count->album_photo_new . '</span>' : '') ?>)
</div>
<?php } ?>
<div class="phdr">
