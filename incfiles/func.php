...
function wfrm($id)
{
    ...
    while ($arrf = mysql_fetch_array($qf)) {
        $whf = mysql_query("select * from `count` where name='" . $arrf['name'] .
            "' order by time desc ;");
...
function dload()
{
    ...
    $fl1 = mysql_query("select `id` from `download` where `time` > '" . $old .
        "' and `type`='file' ;");
...
function fgal($mod = 0)
{
    ...
    global $realtime;
    $old = $realtime - (3 * 24 * 3600);
    $req = mysql_query("select `id` from `gallery` where `time` > '" . $old .
        "' and `type`='ft' ;");
...
function brth()
{
    ...
    $q = mysql_query("select * from `users` where dayb='" . $day . "' and monthb='" .
        $mon . "' and preg='1';");
...
function stlib()
{
    ...
    $countf = mysql_num_rows($fl);
    $old = $realtime - (3 * 24 * 3600);
    $fl1 = mysql_query("select `id` from `lib` where `time` > '" . $old .
        "' and `type`='bk' and `moder`='1';");
    $countf1 = mysql_num_rows($fl1);
    $out = $countf;
    if ($countf1 > 0) {
        $out = $out . '/<font color="#FF0000">+' . $countf1 . '</font>';
    ...
    if ($dostlmod == '1' && ($countm > 0))
        $out = $out . "/<a href='" . $home .
            "/library/index.php?act=moder'><font color='#FF0000'> Мод:$countm</font></a>";
    return $out;
}
...
function wch($id)
{
    ...
    $qf = @mysql_query("select `id` from `users` where  `lastdate` >='" . intval($onltime) .
        "';");
    while ($arrf = mysql_fetch_array($qf)) {
        $whf = mysql_query("select `id` from `count` where `name`='" . $arrf['name'] .
            "' order by `time` desc ;");
        while ($whf1 = mysql_fetch_array($whf)) {
            $whf2[] = $whf1[where];
        }
...
function gbook($mod = 0)
{
    ...
    switch ($mod) {
        case 1:
            $req = mysql_query("SELECT `id` FROM `guest` WHERE `adm`='0' AND `time`>'" . ($realtime -
                86400) . "';");
        ...
        case 2:
            if ($dostmod == 1) {
                $req = mysql_query("SELECT `id` FROM `guest` WHERE `adm`='1' AND `time`>'" . ($realtime -
                    86400) . "';");
                $count = mysql_num_rows($req);
            }
            break;
        ...
        default:
            ...
            if ($dostmod == 1) {
                $req = mysql_query("SELECT `id` FROM `guest` WHERE `adm`='1' AND `time`>'" . ($realtime -
                    86400) . "';");
                $count = $count . '&nbsp;/&nbsp;<span class="red">' . mysql_num_rows($req) .
                    '</span>';
            }
    }
    return $count;
...
function tags($var = '')
{
    ...
    ////////////////////////////////////////////////////////////
    $var = preg_replace_callback('{(?:(\w+://)|www\.|wap\.)[\w-]+(\.[\w-]+)*(?: : \d+)?[^<>"\'()\[\]\s]*(?:(?<! [[:punct:]])|(?<= [-/&+*]))}xis',
        "hrefCallback", $var);
...
function antilink($var)
{
    ...
    ////////////////////////////////////////////////////////////
    $var = eregi_replace("((https?|ftp)://)([[:alnum:]_=/-]+(\\.[[:alnum:]_=/-]+)*(/[[:alnum:]+&._=/~%]*(\\?[[:alnum:]?+&_=/;%]*)?)?)",
        "[реклама]", $var);
...
function trans($str)
{
    ...
    ////////////////////////////////////////////////////////////
    $str = strtr($str, array('a' => 'а', 'b' => 'б', 'v' => 'в', 'g' => 'г', 'd' =>
        'д', 'e' => 'е', 'yo' => 'ё', 'zh' => 'ж', 'z' => 'з', 'i' => 'и', 'j' => 'й',
        'k' => 'к', 'l' => 'л', 'm' => 'м', 'n' => 'н', 'o' => 'о', 'p' => 'п', 'r' =>
        'р', 's' => 'с', 't' => 'т', 'u' => 'у', 'f' => 'ф', 'h' => 'х', 'c' => 'ц',
        'ch' => 'ч', 'w' => 'ш', 'sh' => 'щ', 'q' => 'ъ', 'y' => 'ы', 'x' => 'э', 'yu' =>
        'ю', 'ya' => 'я', 'A' => 'А', 'B' => 'Б', 'V' => 'В', 'G' => 'Г', 'D' => 'Д',
        'E' => 'Е', 'YO' => 'Ё', 'ZH' => 'Ж', 'Z' => 'З', 'I' => 'И', 'J' => 'Й', 'K' =>
        'К', 'L' => 'Л', 'M' => 'М', 'N' => 'Н', 'O' => 'О', 'P' => 'П', 'R' => 'Р', 'S' =>
        'С', 'T' => 'Т', 'U' => 'У', 'F' => 'Ф', 'H' => 'Х', 'C' => 'Ц', 'CH' => 'Ч',
        'W' => 'Ш', 'SH' => 'Щ', 'Q' => 'Ъ', 'Y' => 'Ы', 'X' => 'Э', 'YU' => 'Ю', 'YA' =>
        'Я'));
    return $str;
}
...
function pagenav($var = array())
{
    ...
    if ($start > 0)
        echo '<a href="' . $var['address'] . '&amp;page=' . ($page - 1) .
            '">&lt;&lt;</a> ';
    ...
    if ($page > 13) {
        echo ' <a href="' . $var['address'] . '&amp;page=' . $paa . '">' . $paa .
            '</a> <a href="' . $var['address'] . '&amp;page=' . ($paa + 1) . '">' . ($paa +
            1) . '</a> .. <a href="' . $var['address'] . '&amp;page=' . ($paa * 2) . '">' . ($paa *
            2) . '</a> <a href="' . $var['address'] . '&amp;page=' . ($paa * 2 + 1) . '">' . ($paa *
            2 + 1) . '</a> .. ';
    } elseif ($page > 7) {
        echo ' <a href="' . $var['address'] . '&amp;page=' . $pa . '">' . $pa .
            '</a> <a href="' . $var['address'] . '&amp;page=' . ($pa + 1) . '">' . ($pa + 1) .
            '</a> .. ';
    }
    for ($i = $asd; $i < $asd2; ) {
        if ($i < $var['total'] && $i >= 0) {
    ...
    if ($page2 > 12) {
        echo ' .. <a href="' . $var['address'] . '&amp;page=' . $paa2 . '">' . $paa2 .
            '</a> <a href="' . $var['address'] . '&amp;page=' . ($paa2 + 1) . '">' . ($paa2 +
            1) . '</a> .. <a href="' . $var['address'] . '&amp;page=' . ($paa3) . '">' . ($paa3) .
            '</a> <a href="' . $var['address'] . '&amp;page=' . ($paa3 + 1) . '">' . ($paa3 +
            1) . '</a> ';
    } elseif ($page2 > 6) {
        echo ' .. <a href="' . $var['address'] . '&amp;page=' . $pa2 . '">' . $pa2 .
            '</a> <a href="' . $var['address'] . '&amp;page=' . ($pa2 + 1) . '">' . ($pa2 +
            1) . '</a> ';
    }
    if ($asd2 < $var['total']) {
        echo ' .. <a href="' . $var['address'] . '&amp;page=' . $ba . '">' . $ba .
            '</a>';
    }
...
{
    $str = eregi_replace("((https?|ftp)://)([[:alnum:]_=/-]+(\\.[[:alnum:]_=/-]+)*(/[[:alnum:]+&._=/~%]*(\\?[[:alnum:]?+&_=/%]*)?)?)",
        "<a href='\\1\\3'>\\3</a>", $str);
    return $str;
}
...
function smiles($str)
{
    ...
    while ($file = readdir($dir)) {
        if (ereg(".gif$", "$file")) {
            ...
            $str = str_replace(":$file2", "<img src=\"../sm/prost/$file2.gif\" alt=\"\" />",
                $str);
        }
    }
    closedir($dir);
...
function smilesadm($str)
{
    ...
    while ($file = readdir($dir)) {
        if (ereg(".gif$", "$file")) {
            ...
            $file2 = str_replace(".gif", "", $file2);
            $trfile = trans($file2);
            $str = str_replace(":$file2:", "<img src=\"../sm/adm/$file2.gif\" alt=\"\" />",
                $str);
            $str = str_replace(":$trfile:", "<img src=\"../sm/adm/$file2.gif\" alt=\"\" />",
                $str);
        }
    }
    closedir($dir);
    ...
    for ($a1 = 0; $a1 < $total; $a1++) {
        ...
        while ($k = readdir($d)) {
            if (ereg(".gif$", "$k")) {
                ...
                $file2 = str_replace(".gif", "", $file2);
                $trfile = trans($file2);
                $str = str_replace(":$file2:", "<img src=\"../sm/cat/$a[$a1]/$file2.gif\" alt=\"\" />",
                    $str);
                $str = str_replace(":$trfile:", "<img src=\"../sm/cat/$a[$a1]/$file2.gif\" alt=\"\" />",
                    $str);
            }
        }
        closedir($d);
...
function navigate($adr_str, $itogo, $kol_na_str, $begin, $num_str)
{
    ...
    if ($num_str > 13) {
        echo ' <a href="' . $adr_str . '&amp;page=' . $paa . '">' . $paa .
            '</a> <a href="' . $adr_str . '&amp;page=' . ($paa + 1) . '">' . ($paa + 1) .
            '</a> .. <a href="' . $adr_str . '&amp;page=' . ($paa * 2) . '">' . ($paa * 2) .
            '</a> <a href="' . $adr_str . '&amp;page=' . ($paa * 2 + 1) . '">' . ($paa * 2 +
            1) . '</a> .. ';
    } elseif ($num_str > 7) {
        echo ' <a href="' . $adr_str . '&amp;page=' . $pa . '">' . $pa .
            '</a> <a href="' . $adr_str . '&amp;page=' . ($pa + 1) . '">' . ($pa + 1) .
            '</a> .. ';
    }
    for ($i = $asd; $i < $asd2; ) {
        if ($i < $itogo && $i >= 0) {
    ...
    if ($page2 > 12) {
        echo ' .. <a href="' . $adr_str . '&amp;page=' . $paa2 . '">' . $paa2 .
            '</a> <a href="' . $adr_str . '&amp;page=' . ($paa2 + 1) . '">' . ($paa2 + 1) .
            '</a> .. <a href="' . $adr_str . '&amp;page=' . ($paa3) . '">' . ($paa3) .
            '</a> <a href="' . $adr_str . '&amp;page=' . ($paa3 + 1) . '">' . ($paa3 + 1) .
            '</a> ';
    } elseif ($page2 > 6) {
        echo ' .. <a href="' . $adr_str . '&amp;page=' . $pa2 . '">' . $pa2 .
            '</a> <a href="' . $adr_str . '&amp;page=' . ($pa2 + 1) . '">' . ($pa2 + 1) .
            '</a> ';
    }
    if ($asd2 < $itogo) {
...
{
    $str = strtr($str, array('а' => 'a', 'б' => 'b', 'в' => 'v', 'г' => 'g', 'д' =>
        'd', 'е' => 'e', 'ё' => 'e', 'ж' => 'j', 'з' => 'z', 'и' => 'i', 'й' => 'i', 'к' =>
        'k', 'л' => 'l', 'м' => 'm', 'н' => 'n', 'о' => 'o', 'п' => 'p', 'р' => 'r', 'с' =>
        's', 'т' => 't', 'у' => 'u', 'ф' => 'f', 'х' => 'h', 'ц' => 'c', 'ч' => 'ch',
        'ш' => 'sh', 'щ' => 'sch', 'ъ' => "", 'ы' => 'y', 'ь' => "", 'э' => 'ye', 'ю' =>
        'yu', 'я' => 'ya'));
    return $str;
}
