...
echo '<div class="phdr"><a href="profile.php?user=' . $user['id'] . '"><b>' . $lng['profile'] . '</b></a> | ' . $lng['statistics'] . '</div>' .
    ...
    '<div class="list2">' .
    '<p><h3>' . functions::get_image('rating.png', '', 'class="left"') . '&#160;' . $lng['statistics'] . '</h3><ul>';
if ($rights >= 7) {
...
echo '</ul></p><p>' .
    '<h3>' . functions::get_image('user_edit.png', '', 'class="left"') . '&#160;' . $lng_profile['activity'] . '</h3><ul>' .
    ...
    '</ul></p>' .
    '<p><h3>' . functions::get_image('award.png', '', 'class="left"') . '&#160;' . $lng_profile['achievements'] . '</h3>';
$num = array (
    50,
    100,
    500,
...
foreach ($query as $key => $val) {
    ...
    foreach ($num as $achieve) {
        echo '<td align="center">' . functions::get_image(($user[$key] >= $achieve ? 'green' : 'red') . '.png') . '</td>';
    }
