...
if ($total) {
    ...
    while (($res = mysql_fetch_assoc($req)) !== false) {
        ...
        $link = Vars::$USER_RIGHTS ? '<a href="' . Vars::$SYSTEM_SET['homeurl'] . '/' . Vars::$SYSTEM_SET['admp'] . '/index.php?act=search_ip&amp;mod=history&amp;ip=' . long2ip($res['ip']) . '">' . long2ip($res['ip']) . '</a>' : long2ip($res['ip']);
