...
if ($user['id'] == $user_id && $total < $max_album || $rights >= 7) {
    echo '<div class="topmenu"><a href="album.php?act=edit&amp;id=' . $user['id'] . '">' . $lng_profile['album_create'] . '</a></div>';
}
...
if ($total) {
    while ($res = mysql_fetch_assoc($req)) {
        ...
        echo ($i % 2 ? '<div class="list2">' : '<div class="list1">') .
            ...
            '<a href="album.php?act=show&amp;al=' . $res['id'] . '&amp;id=' . $user['id'] . '"><b>' . checkout($res['name']) . '</b></a>&#160;(' . $count . ')' .
            '<div class="sub">';
        if ($user['id'] == $user_id || $rights >= 6) {
            $menu = array (
                '<a href="album.php?act=sort&amp;mod=up&amp;al=' . $res['id'] . '&amp;id=' . $user['id'] . '">' . $lng['up'] . '</a>',
                '<a href="album.php?act=sort&amp;mod=down&amp;al=' . $res['id'] . '&amp;id=' . $user['id'] . '">' . $lng['down'] . '</a>',
                '<a href="album.php?act=edit&amp;al=' . $res['id'] . '&amp;id=' . $user['id'] . '">' . $lng['edit'] . '</a>',
                '<a href="album.php?act=delete&amp;al=' . $res['id'] . '&amp;id=' . $user['id'] . '">' . $lng['delete'] . '</a>'
            );
