...
if ($img && $user['id'] == $user_id || $rights >= 6) {
    ...
    if (mysql_num_rows($req)) {
        ...
        $album = $res['album_id'];
        echo '<div class="phdr"><a href="album.php?act=show&amp;al=' . $album . '&amp;id=' . $user['id'] . '"><b>' . $lng['photo_album'] . '</b></a> | ' . $lng_profile['image_delete'] . '</div>';
        ...
        if (isset($_POST['submit'])) {
            ...
            header('Location: album.php?act=show&al=' . $album . '&id=' . $user['id']);
        } else {
            echo '<div class="rmenu"><form action="album.php?act=image_delete&amp;img=' . $img . '&amp;id=' . $user['id'] . '" method="post">' .
