...
if (Vars::$USER_RIGHTS == 9 && $do == 'clean') {
    if (isset($_GET['yes'])) {
...
$settings = unserialize(Vars::$SYSTEM_SET['karma']);
...
echo '<form action="index.php?act=karma" method="post"><div class="menu">' .
     ...
     '</form><div class="phdr">' . (Vars::$USER_RIGHTS == 9 ? '<a href="index.php?act=karma&amp;do=clean">' . $lng['karma_reset'] . '</a>' : '<br />') . '</div>' .
